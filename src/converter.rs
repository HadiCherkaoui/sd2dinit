...
pub fn convert(unit: &SystemdUnit, config: &Config) -> Result<ConversionResult, ConvertError> {
    ...
    // Collect EnvironmentFile specs before building the command — needed to
    // decide whether to generate a sh wrapper.
    // Each entry is (optional, path): optional=true when the systemd directive
    // had a leading '-' (don't fail if the file is missing).
    let env_file_specs: Vec<(bool, String)> = unit
        .get_all("Service", "EnvironmentFile")
        .iter()
        .map(|val| match val.strip_prefix('-') {
            Some(p) => (true, p.to_string()),
            None    => (false, val.to_string()),
        })
        .collect();

    // Command — replace known specifiers, then decide on env-var handling.
    // When ExecStart references $VAR AND there are EnvironmentFile entries,
    // we generate a sh wrapper script that sources the files (preserving shell
    // quoting semantics) and exec's the command. Dinit's own env-file parser
    // does not handle shell syntax like single-quoted values or complex $VAR.
    // When there are no EnvironmentFile entries, use dinit's native $/VAR
    // word-splitting form instead.
    let raw_cmd = replace_specifiers(exec_start, &name, &mut warnings);
    let has_env_refs = raw_cmd.contains('$');

    let (command, env_wrapper_script) = if has_env_refs && !env_file_specs.is_empty() {
        let wrapper = generate_env_wrapper(&env_file_specs, &raw_cmd);
        let wrapper_path = config.output_dir.join(format!("{}-env.sh", name));
        let wrapper_cmd = format!("/bin/sh {}", wrapper_path.display());
        warnings.push(Warning {
            directive: "EnvironmentFile".into(),
            message: "shell-format EnvironmentFile detected — generating sh wrapper script for env expansion".into(),
            severity: Severity::Info,
        });
        (wrapper_cmd, Some(wrapper))
    } else {
        (convert_env_refs(raw_cmd), None)
    };
    ...
    // Environment — when a wrapper is generated the EnvironmentFile entries are
    // handled by the wrapper script, so convert_environment skips them.
    let (env_files, env_file_content) = convert_environment(unit, config, &name, &mut warnings, env_wrapper_script.is_some());
...
/// Generates a sh wrapper script that sources `EnvironmentFile=` entries and
/// exec's the original command, preserving shell quoting semantics that dinit's
/// env-file parser does not support.
fn generate_env_wrapper(env_file_specs: &[(bool, String)], cmd: &str) -> String {
    let mut s = String::from("#!/bin/sh\n");
    s.push_str("# Generated by sd2dinit — sources EnvironmentFile(s) via sh and exec's the\n");
    s.push_str("# service command. Required because the env-file uses shell quoting syntax\n");
    s.push_str("# (e.g. single-quoted values) that dinit's env-file parser does not handle.\n");
    for (optional, path) in env_file_specs {
        if *optional {
            s.push_str(&format!("[ -f '{}' ] && . '{}'\n", path, path));
        } else {
            s.push_str(&format!(". '{}'\n", path));
